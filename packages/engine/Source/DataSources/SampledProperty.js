...
function SampledProperty(type, derivativeTypes) {
  ...
  this._lastInterpolationAlgorithm = LinearApproximation;
...
SampledProperty.prototype.getValue = function (time, result) {
  ...
  if (index < 0) {
    ...
    if (index === 0) {
      ...
      linearExtrapolate = this._backwardExtrapolationType === ExtrapolationType.LINEAR_EXTRAPOLATE;
    }
    ...
    if (index >= timesLength) {
      ...
      linearExtrapolate = this._forwardExtrapolationType === ExtrapolationType.LINEAR_EXTRAPOLATE;
    }

    const xTable = this._xTable;
    const yTable = this._yTable;
    var interpolationAlgorithm;
    var interpolationDegree;
    if (linearExtrapolate) {
      interpolationAlgorithm = Cesium.LinearApproximation;
      interpolationDegree = 1;
    } else {
      interpolationAlgorithm = this._interpolationAlgorithm;
      interpolationDegree = this._interpolationDegree;
    }
    if (this._lastInterpolationAlgorithm != interpolationAlgorithm) {
      this._lastInterpolationAlgorithm = interpolationAlgorithm;
      this._updateTableLength = true;
    }
